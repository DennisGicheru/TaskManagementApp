    ...
    <div class="taskEditor">
        <form @submit="editTask">
            ...
            <!-- <div class="form-group flex flex-col bg-white m-auto w-4/5 p-5">
                <label class="mx-auto w-full text-base font-serif  p-2 border">Title: <input type="text" class="pl-2 border hover:border-black-500 w-2/3" v-model="task.title"/></label>
...
export default {
    ...
    methods: {
        editTask(id){
            this.$store.dispatch('editTodos', {id}).then(this.$router.push("/"))
        }
    },
    emits: ['']
}
